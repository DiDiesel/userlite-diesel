...
if( $_SERVER['REQUEST_METHOD'] == 'POST' ) {
	if( $_POST['quick-contact-form-name'] != '' AND $_POST['quick-contact-form-email'] != '' AND $_POST['quick-contact-form-message'] != '' ) {
		...
		$subject = 'New Message From Advertise with Us Form';

		$botcheck = $_POST['quick-contact-form-botcheck'];

		$toemail = 'jim@dieselsellerz.com'; // Your Email Address
		...
		if( $botcheck == '' ) {
			...
			if( $sendEmail == true ):
				...
				echo 'Email <strong>could not</strong> be sent due to an Unexpected Error. Please try again later.<br /><br /><strong>Reason:</strong><br />' . $mail->ErrorInfo . '';
			endif;
		} else {
			echo 'Bot <strong>Detected</strong>.! Clean yourself Botster.!';
		}
	} else {
		echo 'Please <strong>fill in</strong> all the fields and try again.';
	}
} else {
